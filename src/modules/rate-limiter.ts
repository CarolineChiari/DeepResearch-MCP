...
export class ResearchRateLimiter {
  ...
  constructor(config: RateLimitConfig, logger: Logger) {
    ...
    this.logger.info('Rate limiter initialized with in-memory storage', {
      ...
      dailyCostLimit: config.daily_cost_limit_usd
    });
  }
  ...
  private getHourlyLimit(accuracyLevel: "high" | "medium"): number {
    // For simplicity, use a fixed ratio of the daily limits
    return accuracyLevel === 'high' 
      ? Math.ceil(this.config.high_accuracy_daily_limit / 8) // Spread over 8 hours
      : Math.ceil(this.config.medium_accuracy_daily_limit / 8);
  }
  ...
  private getDailyLimit(accuracyLevel: "high" | "medium"): number {
    return accuracyLevel === 'high'
      ? this.config.high_accuracy_daily_limit
      : this.config.medium_accuracy_daily_limit;
  }
...
export function createDefaultRateLimitConfig(): RateLimitConfig {
  return {
    ...
    requests_per_day: parseInt(process.env.MAX_REQUESTS_PER_DAY || '20'),
